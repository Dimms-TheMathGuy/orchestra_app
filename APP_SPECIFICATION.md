...
#### AI Note Taker Rules (Schema-Aware)
* Agent harus memahami schema database Notion template yang dipilih terlebih dahulu sebelum menganalisis transcript.
* Agent harus otomatis diarahkan untuk membaca file `.vtt` dari hasil meeting Zoom sebagai sumber utama.
* Agent harus menghasilkan draft JSON yang selaras dengan konteks template Notion. Contoh: jika template memuat Project Tracker, Task, Issues, dan Report, maka output hanya berisi struktur tersebut.
* Agent harus melakukan tahap "schema acknowledgement" (konfirmasi field dan tipe yang valid) sebelum ekstraksi isi meeting untuk meminimalkan JSON yang salah format/field.
* Output AI bersifat draft dan wajib menunggu approval user sebelum sinkronisasi ke Notion.
